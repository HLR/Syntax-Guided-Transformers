...
from src.train_mvlm import *
from src.model_dual_mvlm import *

from src.dataloader_primitives import *
from src.train_target_primitives import *
...
#     Create directories    
    if config.mode in ['train', 'train_target_simple', 'train_mcnn','train_mvlm','train_primitives']:
...
#         Loading train and val dataloaders
        ...
        command_lang, action_lang = None, None
        if config.load_weights:
            logger.info(f"config.load_weights {str(config.load_weights)}")
            # model.load_state_dict(torch.load(os.path.join("./models/",config.load_weights, 'model.pt'), map_location=device)['model_state_dict'],strict=False,)
            # model.load_state_dict(torch.load(os.path.join("./models/",config.load_weights, 'model.pt'), map_location=device)['model_state_dict'],strict=False,)
            command_lang = torch.load(os.path.join("./models/",config.load_weights, 'command_lang.pth'))
            action_lang = torch.load(os.path.join("./models/",config.load_weights, 'action_lang.pth'))
...
#         Save generated vocab for command and action language
        command_lang, action_lang = prepareData('command', 'action', train_dataloader, include_target=config.include_target,command_lang=command_lang, action_lang=action_lang)
...
#         Building Model
        ...
        if config.load_weights:
            if os.path.exists(os.path.join("./models/",config.load_weights, 'target_predictor_-1.pt')):
                model_address = os.path.join("./models/",config.load_weights, 'target_predictor_-1.pt')
            if os.path.exists(os.path.join("./models/",config.load_weights, 'model.pt')):
                model_address = os.path.join("./models/",config.load_weights, 'model.pt')
            inc_keys = model.load_state_dict(torch.load(model_address, map_location=device)['model_state_dict'],strict=False,)
...
#         Initialize loss and accuracy
        ...
        torch.save({
            ...
            'epoch_train_loss': 1e100,
            'epoch_val_loss': 1e100,
            'epoch_val_acc': 0,
            'max_val_acc': 0,
            'min_val_loss':  1e100,
            'best_epoch': 0
...
#             Train one epoch
            ...
            if config.curriculum_learning:
                if "para" in config.data_path:
                    limit_set = [10, 20, 30, 40, 50, 60, 70] # [10, 20, 30, 35, 40, 45, 50, 60, 70]
                else:
                    limit_set = [5,10,15,20,25,40]
                new_limit = limit_set[min(int(epoch // 10), len(limit_set) - 1)]
                train_dataloader.dataset.set_limit(new_limit)
                logger.info('Command Lenght Limit: {}'.format(new_limit))
...
#     Training (Action Sequence Generation)
    if config.mode == 'train_mvlm':       
        log_file = os.path.join(config.log_path, 'log_train_mvlm.txt')
        logger = get_logger(run_name, log_file, logging.DEBUG)
        writer = SummaryWriter(config.board_path)
...
#         Building Model
        ...
        config.vocab_size += 1
        if config.num_of_streams == 'dual':
            model = MaskedMultiModalModel_Dual(config).to(device)
...
#             Train one epoch
            ...
            epoch_train_loss = train_mvlm_one_epoch(model, train_dataloader, command_lang, action_lang, 
                                               optimizer, epoch, device, config, logger, writer)
...
#             Validate one epoch
            ...
            epoch_val_loss, epoch_val_acc = validate_mvlm_one_epoch(model, val_dataloader, command_lang, 
                                                               action_lang, device, config, logger)
...
#                 Test
                test_acc, _ = validate_mvlm_one_epoch(model, test_dataloader, command_lang, 
                                              action_lang, device, config, logger, exact_match=True)
...
#             config.wandb_run_name = wandb_run_name
        ...
        device = gpu_init_pytorch(device)
        logger.debug('Device: {}'.format(device))
        logger.info('\n')
...
#                 wandb.log({wandb_label[index]: test_acc})
    ...
    # Training Target Predictor
    if config.mode == 'train_primitives':       
        log_file = os.path.join(config.log_path, 'log_train_target_'+str(config.target_layer)+'.txt')
        logger = get_logger(run_name, log_file, logging.DEBUG)
        writer = SummaryWriter(config.board_path)
...
#         Loading train and val dataloaders
        logger.debug('Train Data Path: {}'.format(train_data_path))
        logger.info('Loading Train Data ..')
        train_dataloader = dataloader_primitives(train_data_path, device, batch_size=config.batch_size)
        
        logger.debug('Val Data Path: {}'.format(val_data_path))
        logger.info('Loading Val Data ..')
        val_dataloader = dataloader_primitives(val_data_path, device, batch_size=config.batch_size)
...
#         Save generated vocab for command and action language
        command_lang, action_lang = prepareData('command', 'action', train_dataloader, include_target=config.include_target,exclude_lang2=True)
        torch.save(command_lang, os.path.join(config.model_path, 'command_lang.pth'))
        torch.save(action_lang, os.path.join(config.model_path, 'action_lang.pth'))
        logger.debug('Command Language: {}'.format(command_lang.word2index))
        logger.debug('Action Language: {}'.format(action_lang.word2index))
...
#         Building Model
        ...
        if config.num_of_streams == 'dual':
            ...
            model = MultiModalModel_Single(config).to(device)
        target_predictor = PrimitivesTargetPredictor(model,config).to(device)
        config.num_params = sum(p.numel() for p in target_predictor.parameters())
        out_model_archi(target_predictor, config.num_params, config)
...
#             Train one epoch
            ...
            epoch_train_loss = train_one_epoch_target_primtives(target_predictor, train_dataloader, command_lang, action_lang, 
                                               optimizer, epoch, device, config, logger, writer)
...
#             Validate one epoch
            ...
            epoch_val_loss, epoch_val_acc, _ = validate_one_epoch_target_primtives(target_predictor, val_dataloader, command_lang, 
                                                               action_lang, device, config, logger)
...
#             Loading model
            if config.test_epoch == -1:
                logger.info('Loading Model From: {}'.format(os.path.join(config.model_path, 'target_predictor_'+str(config.target_layer)+'.pt')))
                target_predictor = PrimitivesTargetPredictor(model, config, target_layer=config.target_layer).to(device)
                checkpoint = torch.load(os.path.join(config.model_path, 'target_predictor_'+str(config.target_layer)+'.pt'), map_location=device)
                target_predictor.load_state_dict(checkpoint['model_state_dict'])

            test_data_paths = []

            train_fname = 'train_spatial.json'
            val_fname = 'dev_spatial.json'
            test_fname = 'test_spatial.json'
            ...
            if config.dataset == 'gscan':
                ...
                if config.test_split == 'comp':
                    for split in ['visual', 'visual_easier', 
                                  ...
                                  'adverb_1', 'adverb_2', 'contextual']:
                        split_path = os.path.join(gscan_data_path, 'gSCAN-compositional_splits') 
                        test_data_paths.append(os.path.join(split_path, split+'_spatial.json'))
...
#                 Load test data
                ...
                logger.debug('Test Data Path: {}'.format(test_path))
                logger.info('Loading Test Data ..')
                test_dataloader = dataloader_primitives(test_path, device, batch_size=config.batch_size, random_shuffle=False)                       
...
#                 Test
                _, test_acc, _ = validate_one_epoch_target_primtives(target_predictor, test_dataloader, command_lang, 
                                              action_lang, device, config, logger)
