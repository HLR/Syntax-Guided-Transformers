...
class PrimitivesTargetPredictor(nn.Module):

    def __init__(self, model, config, target_layer=-1, random_layer=False):
        super(PrimitivesTargetPredictor, self).__init__()
        ...
        self.seq_length = 50 + 37
        self.classifier = nn.Linear(self.embed_size*2, 36)
...
#         Concatenating encoder output for language and visual stream
        ...
        # memory = memory.reshape(memory.shape[0], self.embed_size*self.seq_length)
        # if self.random_layer:
        #     memory = torch.rand(memory.shape).to('cuda:0')
        memory = torch.cat((sequence_output_t[:,0,:], sequence_output_v[:,0,:]), 1)
