...
from aux_data import make_aux_data

file_paths_reascan = [
                      '../../data-np-vp/ReaSCAN-v1.1/ReaSCAN-compositional/data-compositional-splits.txt', 
                      '../../data-np-vp/ReaSCAN-v1.1/ReaSCAN-compositional-a1/data-compositional-splits.txt',
                      '../../data-np-vp/ReaSCAN-v1.1/ReaSCAN-compositional-a2/data-compositional-splits.txt',
                      '../../data-np-vp/ReaSCAN-v1.1/ReaSCAN-compositional-a3/data-compositional-splits.txt',
                      '../../data-np-vp/ReaSCAN-v1.1/ReaSCAN-compositional-b1/data-compositional-splits.txt',
                      '../../data-np-vp/ReaSCAN-v1.1/ReaSCAN-compositional-b2/data-compositional-splits.txt',
                      '../../data-np-vp/ReaSCAN-v1.1/ReaSCAN-compositional-c1/data-compositional-splits.txt',
                      '../../data-np-vp/ReaSCAN-v1.1/ReaSCAN-compositional-c2/data-compositional-splits.txt',]

file_paths_gscan = ['../../data-np-vp/ReaSCAN-v1.1/gSCAN-compositional_splits/dataset.txt']

file_paths_google = ['../../data-np-vp/spatial_relation_splits/dataset.txt']


class DKConsituencyParser:
    def __init__(self):
        import stanza
        self.model = stanza.Pipeline(lang='en', processors='tokenize,pos,constituency',download_method=None)
    ...
    def parse(self, text):
        ...
        doc = self.model(text)
        import re
        parsed = str(doc.sentences[0].constituency).replace("("," ( ").replace(")"," ) ")
        parsed = str(re.sub(' +', ' ',parsed)).strip()
        return ",".join(parsed.split(" "))

text_parser = DKConsituencyParser()
    ...
    with open(file_path, 'r') as infile:
        ...
        for split in splits:
            ...
            for data_example in tqdm(all_data["examples"][split]):
                ...
                input_command = text_parser.parse(" ".join(input_command)).split(',')
        ...
        for split, dt in data.items():
            ...
            if args.dataset == 'reascan':
                ...
                with open(file_path.split('dataset')[0] + split + '.json', 'w') as f:
                    for line in dt:
                        f.write(json.dumps(line) + '\n')
    make_aux_data('../../data-np-vp/ReaSCAN-v1.1/')