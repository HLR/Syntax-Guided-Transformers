...
def calc_aux_loss(aux_loss_fn, batch_text_mask, all_attention_wts_b4_dropout):
    total_loss = 0
    weights = []
...
def train_one_epoch(model, train_dataloader, command_lang, action_lang, 
    ...
    for batch_index, data in tqdm(enumerate(train_dataloader)):
        ...
        if config.num_of_streams == 'dual':
            action_pred, encoded_layers_t, encoded_layers_v, all_attention_wts,all_attention_wts_b4_dropout,embedding_output, v_embedding_output = model(batch_text, batch_world, batch_world_loc, 
        ...
        if config.tree_aux_loss:
            loss += float(config.tree_aux_loss) * calc_aux_loss(aux_loss_fn, batch_text_mask, all_attention_wts_b4_dropout) # 0.01
    ...
    with torch.no_grad():
        for batch_index, data in tqdm(enumerate(val_dataloader)):
            batch_text, batch_text_mask = tensorFromBatch(
                ...
                input_mask=data.mask,
                target_data=data.target_location,
            ...
            if not exact_match:
                ...
                loss = loss_fn(action_pred.reshape(-1, action_pred.shape[-1]), batch_target_output.reshape(-1))
                if config.tree_aux_loss:
                    loss += float(config.tree_aux_loss) * calc_aux_loss(aux_loss_fn, batch_text_mask, all_attention_wts_b4_dropout) # 1e-6
                losses += loss.item()      
...
def greedy_decode_dual(model, batch_text, batch_text_mask, batch_world, 
    ...
    batch_prediction = []
    if len(batch_text_mask.shape) == 2:
        extended_attention_mask = batch_text_mask.unsqueeze(1).unsqueeze(2)
    else:
        extended_attention_mask = batch_text_mask.unsqueeze(1)
    # extended_attention_mask = batch_text_mask.unsqueeze(1).unsqueeze(2)
