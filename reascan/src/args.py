...
def build_parser():
    ...
    # Global arguments
    parser.add_argument('--mode', type=str, required=True, choices=['train', 'train_mcnn', 'test', 'train_target_predictor', 'train_target_simple', 'train_mvlm', 'train_primitives'], help='Modes: train, test')
    ...
    parser.add_argument('--curriculum_learning', type=int, required=False)
    parser.add_argument('--load_weights', type=str, required=False)
    ...
    parser.add_argument('--attention_mask', type=int, default=0, choices=[0,1], help='whether do attention mask')
    parser.add_argument('--num_of_streams', type=str, default='dual', choices=['dual', 'single'], help='Num of streams in transformer')
    parser.add_argument('--pos_embed', type=str, default='learned', choices=['learned', 'sincos','relative','gated'], help='Text Position Embedding to use')
    parser.add_argument('--rpe_heads', type=int, default=8, help='Text Relative Position Embedding heads')
    parser.add_argument('--rpe_dropout', type=float, default=0.2, help='Text Relative Position Embedding heads')
    parser.add_argument('--v_pos_embed', type=str, default='linear', choices=['learned', 'sincos','relative'], help='Image Position Embedding to use')
    parser.add_argument('--v_rpe_heads', type=int, default=8, help='Text Relative Position Embedding heads')
    parser.add_argument('--v_rpe_dropout', type=float, default=0.1, help='Text Relative Position Embedding heads')
    
    parser.add_argument('--tree_aux_loss',type=str, default=None, help='whether to use tree embedding aux loss')

    parser.add_argument('--dependency_mask', help='whether to use dependency_mask')
    parser.set_defaults(dependency_mask=False)

    parser.add_argument('--constituency_mask', help='whether to use constituency_mask')
    parser.set_defaults(constituency_mask=False)

    parser.add_argument('--self_att_mask_upper', help='whether to use self_att_mask_upper',type=int, default=1)

    parser.add_argument('--self_att_mask_lower', help='whether to use self_att_mask_lower',type=int, default=1)

    parser.add_argument('--self_att_mask_diag', help='whether to use self_att_mask_diag',type=int, default=1)
    # Experiment arguments
    parser.add_argument('--data_version', type=str, default='v1', choices=['v1', 'v2', 'v3', 'v4'], help='Data Version for Add C1 C2 Experiment')
    
    # Weight Sharing Experiment
    parser.add_argument('--share_v_layers', type=int, default=0, help='share visual encoder layers')
    parser.add_argument('--share_l_layers', type=int, default=0, help='share language encoder layers')
    parser.add_argument('--share_x_layers', type=int, default=0, help='share cross attention encoder layers')
